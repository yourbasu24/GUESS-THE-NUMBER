            ...
            if guess == generated:
                ...
                if guess < generated:
                    if guess< generated-10:
                        ...
                        #emp.pack(padx=10,pady=10)
                        messagebox.showinfo("oops"," Too low! Guess higher ")
                        threading.Thread(target=speak,args=("Too low! Guess higher",)).start()
                        
                        
                    else:
                        messagebox.showinfo("oops"," so close!! guess higher ")
                        threading.Thread(target=speak,args=("so close!! guess higher",)).start()
                    
                else:
                    if guess>generated+10:
                        messagebox.showinfo("oops"," Too high! Guess lower ")
                        threading.Thread(target=speak,args=("Too high! Guess lower.",)).start()
                        #temp.destroy()
                    else:
                        messagebox.showinfo("oops"," so close!! guess lower ")
